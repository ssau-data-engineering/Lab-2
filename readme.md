...
При построении первого пайплайна главной проблемой было запустить и настроить контейнеризацию докер в докере после того как это было успешно выполнено задача построения пайплайна довольно просто решалась.

В качестве пояснения к получившемуся пайплайну отмечаю, что для задачи был создан докер [образ](./Dockerfile) и размещен на докер хаб https://hub.docker.com/repository/docker/ermkeg/lab2/general для возможности доступа из среды Airflow. Так как в изначально коде предлагаемой архитектуры пайплайна предполагались к использованию отдельные контейнеры для каджого этапа, но создавал я только один, в целях экономии времени на загрузку и развертываение контейнера, операции выполняющиеся в виде питон файла были объединены, так пайплай получился чуть короче.

![Пайплайн-1](./opera_byks9xXLMS.png)

Конвертация видео в аудио была выполненена с помощью предлагаемой утилиты ffmpeg, трансформация аудио в тектс выполнялась через упакованную в отдельный питон package сети whisper, а суммаризация полученного текста с помощью модели Bart в виде имплементации пайплайна в transfromers. Итоговый экспорт полученного файла в пдф был сделан с помощью fpdf.

Для второго пайплайна была обучена тестовая модель выполняющая классификацию на датасете MNIST, опять же элементы пайплайна котороые молги быть выполнены в одном файле были объединены, и выполнялись в контейнере [tensorflow](https://hub.docker.com/r/tensorflow/tensorflow/)

Итоговый пайплайн представлен на рисунке:

![Пайплайн-2](./opera_nXapm8QBSX.png.png)

А получившаяся модель экспортирована в формате tensorflow [модели](./saved_model.pb). Так же во время обучения происходила запись [логов](./log.csv) с каждой эпохи.
